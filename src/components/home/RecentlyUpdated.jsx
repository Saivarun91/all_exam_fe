...
import RecentlyUpdatedClient from "./RecentlyUpdatedClient";
...
export default async function RecentlyUpdated() {
  const [sectionSettings, exams] = await Promise.all([
    getSectionSettings(),
    getExams(),
  ]);
  ...
  return (
    <RecentlyUpdatedClient sectionSettings={sectionSettings} exams={exams} />
  );
}