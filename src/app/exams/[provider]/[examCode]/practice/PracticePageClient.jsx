...
// export default function PracticePageClient({ exam, practiceTests, topics, faqs, testimonials, provider, examCode, breadcrumbItems }) {
//   const router = useRouter();
//   const [showLoginModal, setShowLoginModal] = useState(false);
//   const [pendingTestUrl, setPendingTestUrl] = useState(null);
...
//     const cleanSlug = typeof testIdentifier === "string" ? testIdentifier.replace(/-[a-f0-9]{8}$/i, "") : testIdentifier;
...
//                 <>
//                   <BreadcrumbLink asChild>
//                     <Link href={item.url} className="text-[#0C1A35] hover:text-[#1A73E8]">{item.name}</Link>
//                   </BreadcrumbLink>
//                   <BreadcrumbSeparator />
//                 </>
//               ) : (
...
//       <div className="mb-8">
//         <div className="flex flex-wrap gap-2 mb-4">
//           <Badge className="bg-[#1A73E8] text-white border-0">{exam.code}</Badge>
//           <Badge className="bg-[#1A73E8]/10 text-[#1A73E8] border-0">{exam.provider}</Badge>
//           {exam.category.map((cat, idx) => (
//             <Badge key={idx} className="bg-[#1A73E8]/10 text-[#1A73E8] border-0">{cat}</Badge>
//           ))}
//           <Badge className="bg-green-100 text-green-700 border-0">{exam.difficulty}</Badge>
//         </div>
...
//       {practiceTests.length > 0 && (
//         <section className="mb-12">
//           <h2 className="text-3xl font-bold text-[#0C1A35] mb-6">
...
//                           {Number(topic.percentage).toFixed(0)}%
//                         </span>

//                       </div>
//                       <Progress
//                         value={Math.min(Number(topic.percentage) || 0, 100)}
//                         className="h-2"
//                       />
//                     </div>
//                   </div>
...
//             <div className="text-center p-4 border border-[#DDE7FF] rounded-lg">
//               <BookOpen className="w-8 h-8 text-[#1A73E8] mx-auto mb-2" />
//               <p className="text-2xl font-bold text-[#0C1A35]">{exam.totalQuestions}</p>
//               <p className="text-sm text-[#0C1A35]/60">Questions</p>
//             </div>
//             <div className="text-center p-4 border border-[#DDE7FF] rounded-lg">
//               <Clock className="w-8 h-8 text-[#1A73E8] mx-auto mb-2" />
...
//             <div className="text-center p-4 border border-[#DDE7FF] rounded-lg">
//               <Target className="w-8 h-8 text-[#1A73E8] mx-auto mb-2" />
//               <p className="text-2xl font-bold text-[#0C1A35]">{exam.passingScore}</p>
//               <p className="text-sm text-[#0C1A35]/60">Passing Score</p>
//             </div>
//             <div className="text-center p-4 border border-[#DDE7FF] rounded-lg">
//               <Award className="w-8 h-8 text-[#1A73E8] mx-auto mb-2" />
...
//                       <Star key={i} className="w-4 h-4 fill-yellow-400 text-yellow-400" />
//                     ))}
//                     {testimonial.verified && <Badge className="bg-green-100 text-green-700 border-0 text-xs">Verified</Badge>}
//                   </div>
//                   <p className="text-sm text-[#0C1A35]/80 leading-relaxed">"{testimonial.review}"</p>
//                 </CardContent>
//               </Card>
//             ))}
//           </div>
...
//             {faqs.map((faq, idx) => (
//               <AccordionItem key={idx} value={`item-${idx}`} className="border-[#DDE7FF]">
//                 <AccordionTrigger className="text-[#0C1A35] hover:no-underline">{faq.question}</AccordionTrigger>
//                 <AccordionContent className="text-[#0C1A35]/80">
//                   <div className="tiptap-editor-content" dangerouslySetInnerHTML={{ __html: faq.answer }} />
//                 </AccordionContent>
//               </AccordionItem>
//             ))}
//           </Accordion>
...
//           </DialogHeader>
//           <div className="flex gap-3 mt-4">
//             <Button variant="outline" onClick={() => setShowLoginModal(false)} className="flex-1">Cancel</Button>
//             <Button
...
export default function PracticePageClient({
  ...
  // Listen for login events to redirect user
  useEffect(() => {
    const handleLogin = () => {
      if (pendingTestUrl) {
        setShowLoginModal(false);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-3 lg:px-1 py-4">
      ...
      {practiceTests?.length > 0 && (
        <section className="mb-12">
          <h2 className="text-3xl font-bold text-[#0C1A35] mb-6">Practice Tests Included</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {practiceTests.map((test, idx) => (
