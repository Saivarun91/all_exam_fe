...
export default function ValuePropositionsClient({ section, features }) {
  ...
  return (
    <div className="container mx-auto px-4">
      ...
      <h2 className={`text-2xl sm:text-3xl md:${section.heading_font_size || "text-4xl"} ${section.heading_font_family || "font-bold"} ${section.heading_color || "text-[#0C1A35]"} text-center mb-3 md:mb-4 px-2`}>
        {section.heading}
      </h2>

      <p className={`text-center ${section.subtitle_color || "text-[#0C1A35]/70"} text-sm sm:text-base md:${section.subtitle_font_size || "text-lg"} mb-8 md:mb-12 max-w-2xl mx-auto px-2`}>
        {section.subtitle}
      </p>
      ...
      <div className="relative">
        ...
        {features.length > itemsPerView && (
          <>
            <Button
              ...
              disabled={currentIndex === 0}
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 rounded-full w-12 h-12 min-h-[44px] min-w-[44px] bg-white text-[#1A73E8] shadow-lg border"
              size="icon"
            ...
            <Button
              ...
              disabled={currentIndex >= maxIndex}
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 rounded-full w-12 h-12 min-h-[44px] min-w-[44px] bg-white text-[#1A73E8] shadow-lg border"
              size="icon"
          ...
          >
            {features.map((feature, index) => {
              ...
              return (
                ...
                >
                  <Card className="border-[#D3E3FF] bg-white hover:shadow-lg h-full">
                    <CardContent className="p-6 text-center space-y-4">
        ...
        {features.length > itemsPerView && (
          <div
            ...
            aria-label="Value propositions carousel slides"
          >
            {Array.from({ length: maxIndex + 1 }).map((_, idx) => (
              <button
                type="button"
                key={idx}
                onClick={() => setCurrentIndex(idx)}
                aria-label={`Go to value propositions slide ${idx + 1} of ${maxIndex + 1}`}
